...
using Sugcon.Feature.Basic.Rendering.Extensions;
using Sugcon.Feature.Navigation.Rendering.Extensions;
using Sugcon.Feature.Products.Rendering.Extensions;
using Sugcon.Feature.Services.Rendering.Extensions;
...
using Sugcon.Feature.Articles.Rendering.Extensions;
using Sugcon.Foundation.GrapheNetor.Rendering;
using Statiq.Common;
using Statiq.App;
using Sugcon.Foundation.GrapheNetor.Rendering.Extension;
using Sugcon.Feature.Authors.Rendering.Extension;

namespace Sugcon.Project.Sugcon.Rendering
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            //Add Graphenetor services & pass required services to bootstrapper
            services.AddFoundationGrapheNetor();
            services.AddScoped<IBootstrapper>((services) =>
            {
                return Statiq.App.Bootstrapper.Factory
                                .CreateDefault(new string[] { })
                                .ConfigureServices((service, setting) =>
                                {
                                    service.AddStackExchangeRedisCache(options =>
                                    {
                                        options.Configuration = RedisConfiguration.ConnectionString;
                                    });
                                    service.AddCachingServices();
                                });
            });
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            app.UseEndpoints(endpoints =>
            {
                ...
                endpoints.MapControllerRoute(
                   "generatepages",
                   "generatepages",
                   new { controller = "Default", action = "GeneratePages" }
               );
